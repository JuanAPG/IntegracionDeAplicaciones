"""
apps/services/escritorio/Ejercicio1_local_demo.py
Copia de Ejercicio1.py para correr y demostrar la pestana "Clasificacion
SOAP" en una maquina SIN PostgreSQL instalado (por ejemplo tu laptop,
cuando el modulo SOAP real solo esta desplegado en la VM). El unico
cambio real es la URL por omision del campo "URL del servicio SOAP":
apunta a mock_soap_server.py (memoria, sin base de datos) en vez de a
un despliegue real. Arranca primero ese servidor:

    python3 mock_soap_server.py

y luego esta GUI. El resto del codigo -- incluida toda la logica de
construir el Envelope, leer la respuesta y mostrar un SOAP Fault -- es
identico a Ejercicio1.py: lo que se demuestra aqui es la misma GUI,
no una version reducida.
"""
import re
import socket
import getpass
...
class Ejercicio1(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Clasificador de Servicios Cloud Computing (DEMO LOCAL - sin PostgreSQL)")
