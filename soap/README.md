...
# Copie la carpeta soap/ al servidor (scp o git clone)
sudo cp -r soap /opt/library/soap
...
* El `.env` **no se publica**: esta en `soap/.gitignore`. Compruebelo con
  `git ls-files | grep -i "\.env"` — solo debe salir `soap/.env.example`.
